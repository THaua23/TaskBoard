...
board.addEventListener('dblclick', (event) => {
    ...
    if (event.target === board) {
        ...
        card.addEventListener('dragstart', (event) => {
            ...
            cardWidth = dragCard.offsetWidth
            cardHeight = dragCard.offsetHeight

            trash.classList.add('show-trash-can')
        })

        // remove o erro de transparência do card
        card.addEventListener('dragend', () => {
            dragCard.classList.remove('drag-card')
            trash.classList.remove('show-trash-can')
            trash.classList.remove('animate-trash')
        })
...
trash.addEventListener('dragover', () => {
    trash.classList.add('animate-trash')
})
